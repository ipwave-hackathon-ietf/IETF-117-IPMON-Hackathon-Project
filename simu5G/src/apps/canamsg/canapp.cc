...
{
    EV << "canapp::initialize - stage " << stage << endl;
    ...
    EV << "canapp::initialize - binding to port: local:" << localPort_ << " , dest:" << destPort_ << endl;
...
void canapp::handleMessage(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        if (!strcmp(msg->getName(), "selfSender"))
            ...
            throw cRuntimeError("canapp::handleMessage - Unrecognized self message");
    }
}

void canapp::sendCANAPacket()
{
    Packet* packet = new inet::Packet("CCM"); //alert to ccm

    auto ccm = makeShared<Canaccm>();
    ccm->setSno(nextSno_);
    ccm->setPayloadTimestamp(simTime());
    ccm->setChunkLength(B(size_));
    ccm->addTag<CreationTimeTag>()->setCreationTime(simTime());

    packet->insertAtBack(ccm);

    EV << "canapp::sendCanaccm - Sending message [" << nextSno_ << "]\n";
    ...
    if (stopTime_ <= SIMTIME_ZERO || d < stopTime_) {
        ...
        EV << "canapp::sendCanaccm - Stop time reached, stopping transmissions" << endl;
}

void canapp::refreshDisplay() const
{
    char buf[80];
